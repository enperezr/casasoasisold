...
class Property extends CachedModel implements FeedItem
{
    ...
    public function notification(){
        return $this->hasOne('App\Notification', 'property_id');
    }
    ...
    public function getContact($action = false){
        if(!$this->actions->isEmpty())
            return $this->actions->first()->contact;
    }
     ...
     * @return mixed
     */
    public static function idSearch($gestor, $action, $type, $province, $municipio, $locality, $tstate, $ttype_construction,
                                    $price_min, $price_max, $currency, $condition, $surface_min, $surface_max, $rooms, $baths, $extras,
                                    $order=null, $paginate=null){
        //start query, not doing this can produce that equally named columns on related tables overwrite themselves
        $query = Property::select(['properties.*', 'user_actions.action_id as ac']);
        ...
        function finishSearchAndReturn($query, $order, $paginate){
            $query = $query->with(['typeProperty','province','municipio','locality']);
            $query = $query->orderBy('has_images', 'DESC');
            if(!$order || $order == 'recent')
                $query = $query->orderBy('properties.created_at', 'DESC');
            if($order == 'oldest')
                $query = $query->orderBy('properties.created_at', 'ASC');
            if($order == 'expensive')
        ...
        if($action == '2'){
            ...
            if($currency != 0){
                $query = Helper::addWhereToCommand($query, 'user_actions.currency_id', $currency);
            }
        }
        ...
        if($gestor == 0){
            $query = Helper::addWhereToCommand($query, 'properties.gestor', 'zero');
        }
     ...
     * @return bool|mixed
     */
    public static function search($gestor, $action, $type, $province, $municipio, $locality, $tstate, $ttype_construction,
                                  $price_min, $price_max, $currency, $condition, $surface_min, $surface_max, $rooms, $baths, $extras,
                                  $order=null, $paginate=null){
        //special extras
        if(!empty($extras)){
        ...
        $type = Helper::resolveForQuery(TypeProperty::class, 'sluggedplural', $type);       
        ...
        return self::idSearch($gestor, $action, $type, $provinceId, $municipioId, $localityId, $tstate, $ttype_construction,
            $price_min, $price_max, $currency, $condition, $surface_min, $surface_max, $rooms, $baths, $extras, $order, $paginate);
    }


    public static function countPerAction($gestor=1, $type=null, $province=null, $municipio=null,
                                          $locality=null, $state_contruction=null, $construction_type=null){
     ...
     *      scalar
     */
    public static function countPerAttributes($grouping, $gestor=1, $action=null, $type=null, $province=null, $municipio=null,
                                                $locality=null, $state_contruction=null, $construction_type=null){
        ...
        if(!$gestor)
            $command = Helper::addWhereToCommand($command, 'properties.gestor', $gestor);
    ...
    public function getUrl($action=false){
        ...
        $url.= '/'.$this->typeProperty->sluggedplural;
    ...
    public function getLabelName($action=false){
        if($action)
            return trans('messages.property.action.type.locality', ['locality'=>($this->locality && $this->locality->name != 'unspecified') ? $this->locality->name.', ' : '',
                'municipio'=>$this->municipio->name,
                'province'=>', '.$this->province->name
            ]);
        return trans('messages.property.type.locality', ['locality'=>($this->locality && $this->locality->name != 'unspecified') ? $this->locality->name.', ' : '',
            'municipio'=>$this->municipio->name,
            'province'=>', '.$this->province->name
        ]);
    }
    ...
    public static function getFullPropertiesSinceDate($since = false, $upto=false){
        $command = Property::where('active', 1)->orderBy('updated_at', 'DESC')->with('commodities', 'images', 'actions');
        if($since)
            $command->where('updated_at', '>', $since);
        if($upto)
            $command->where('updated_at', '<', $upto);
        $properties = $command->get();
        return $properties;
    }
    ...
    public function expiredAgo(){
        $days = $this->actions[0]->time;
        return Carbon::today()->diffInDays(Carbon::createFromFormat('Y-m-d H:i:s', $this->date)->addDays($days));
    }
    ...
    public function getRating(){
        ...
        if($images > 20)
            ...
            $v_imgs = ($images*100/20)*.6/100;
        }
        if($description > 300)
            $v_descr = .4;
        else
            $v_descr = ($description*100/300)*.4/100;
        return round($base + $v_descr + $v_imgs, 2);
    }

    public function setOwnerAndSave($user){
    ...
    public function getFeedItemTitle() : string
    {
        ...
        for($i = 0; $i < count($actions); $i++){
            ...
            if($actions[$i]->action_id == 1){
                $title.= $actions[$i]->price ? '$'.$actions[$i]->price : 'Precio Oculto';
            }
            if($actions[$i]->action_id == 2){
                $title.=$actions[$i]->condition;
            }
        }
        $contact = $this->getContact();
        $title.='. '.$contact->names.' '.$contact->phones.'.';
        return $title;
    }
    ...
    public function getFeedItemAuthor() : string
    {
        $contact = $this->getContact();
        if($contact)
            return $contact->names.' '.$contact->phones;
        return 'No contact';
    }
}
